...
| \<A-k>/\<A-j> | Move current line up/down |
| \<C-h>/\<C-j>/\<C-k>/\<C-l> | Move left/down/up/right between split panes |
| \<Tab>/<Down> | Next option in autocomplete |
| \<S-Tab>/<Up> | Previous option in autocomplete |
| \<CR> | Select option in autocomplete |
| \<S-Up>/\<S-Down> | Scroll documentation of selected item |
| \<C-Space> | Attempt autocomplete if hidden |
| \<leader>\<Tab> | Abort/Close autocomplete menu |
